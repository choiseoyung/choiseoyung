...
#         (self.x, self.y) = (x1, y1)
#         if x1 <= 0 or x2 >= WIDTH:                       # 공의 x좌표가 음수거나, x좌표가 오른쪽경계를 넘으면
#             self.xspeed = -self.xspeed                   # 속도의 부호를 반전시킨다.
#         if y1 <= 0 or y2 >= HEIGHT:                      # 공의 y좌표가 음수거나, y좌표가 오른쪽경계를 넘으면
...
# canvas.bind("<KeyRelease>", keyRelease)     # 키보드를 풀어줬을 때 이벤트처리
...
#                 self.id = self.canvas.create.image(x, y, anchor=NW, image=self.img)
#             else:
...
#             y2 = y1+100
#         else:
#             (x1, y1, x2, y2) = self.canvas.coords(self.id)  # 공의 위치를 얻는다.
#             (self.x, self.y) = (x1, y1)
...
# canvas.bind("<KeyPress-Up>", keyPress_UP)   # 키보드 위쪽방향키 눌림 이벤트에 함수를 연결한다.
# canvas.bind("<KeyPRESS-Down>", keyPress_Down)  # 키보드 아래쪽방향키 눌림 이벤트에 함수를 연결한다.
# canvas.bind("<KeyRelease>", keyRelease)   # 키보드를 풀어줬을 때 이벤트 처리
...
#         bullet.move()
        ...
        #     bullets.remove(bullet)
        # 적을 맞추면 점수가 올라간다.
    #     if bullet.x > enemy.x and bullet.x < enemy.x+100 and bullet.y > enemy.y and bullet.y < enemy.y+100:
    #         score = getScore(score)
    # enemy.move()
    # spaceship.move()
    # window.update()
